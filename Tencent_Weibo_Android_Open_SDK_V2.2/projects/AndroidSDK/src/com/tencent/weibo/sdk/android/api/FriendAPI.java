...
public class FriendAPI extends BaseAPI {
	 ...
	 * 互听关系url
	 */
	private static final String SERVER_URL_MUTUALLIST = API_SERVER
			+ "/friends/mutual_list";
	/**
	 * 收听某个用户url
	 * */
	 ...
	 * 我收听的人列表url
	 * */
	private static final String SERVER_URL_IDOLLIST = API_SERVER
			+ "/friends/idollist";
	/**
	 * 我的听众列表url
	 * */
	private static final String SERVER_URL_FANSLIST = API_SERVER
			+ "/friends/fanslist";
	/**
	 * 检测是否我的听众或收听的人url
	 * */
	private static final String SERVER_URL_CHECK = API_SERVER
			+ "/friends/check";
	/**
	 * 获取最近联系人列表url
	 * */
	private static final String SERVER_URL_GetINTIMATEFRIENDS = API_SERVER
			+ "/friends/get_intimate_friends";
	 ...
	 *            请求个数(1-30)
	 * @param install
	 *            过滤安装应用好友（可选） 0-不考虑该参数，1-获取已安装应用好友，2-获取未安装应用好友
	 * @param mCallBack
	 *            回调函数
	 * @param mTargetClass
	 ...
	 */
	public void getMutualList(Context context, String format, String name,
			String fopenid, int startindex, int reqnum, int install,
	 ...
	 * @param fopenids
	 *            你需要读取的用户openid列表，用下划线“_”隔开，例如：
	 *            B624064BA065E01CB73F835017FE96FA_B624064BA065E01CB73F835017FE96FB
	 *            （可选，最多30个） name和fopenids至少选一个，若同时存在则以name值为主
	 * @param mCallBack
	 *            回调函数
	 * @param mTargetClass
	 *            返回对象类，如果返回json则为null
	 ...
	 *            BaseVO.TYPE_BEAN_LIST=3 BaseVO.TYPE_JSON=4
	 */
	public void addFriend(Context context, String format, String name,
	 ...
	 *            请求个数(1-30)
	 * @param startindex
	 *            起始位置（第一页:填0，继续向下翻页：填【reqnum*（page-1）】）
	 * @param mode
	 *            获取模式，默认为0 mode=0，旧模式，新粉丝在前，只能拉取1000个
	 ...
	 *            BaseVO.TYPE_BEAN_LIST=3 BaseVO.TYPE_JSON=4
	 */
	public void friendIDolList(Context context, String format, int reqnum,
			int startindex, int mode, int install, HttpCallback mCallBack,
			Class<? extends BaseVO> mTargetClass, int resultType) {
		ReqParam mParam = new ReqParam();
		mParam.addParam("scope", "all");
		mParam.addParam("clientip", Util.getLocalIPAddress(context));
	 ...
	 *            请求个数(1-30)
	 * @param startindex
	 *            起始位置（第一页填0，继续向下翻页：填上一次请求返回的nextstartpos）
	 * @param mode
	 *            获取模式，默认为0 mode=0，旧模式，新粉丝在前，只能拉取1000个
	 *            mode=1，新模式，最多可拉取一万粉丝，暂不支持排序
	 * @param install
	 *            过滤安装应用好友（可选） 0-不考虑该参数，1-获取已安装应用好友，2-获取未安装应用好友
	 * @param sex
	 *            按性别过滤标识 1-男，2-女，0-不进行性别过滤，默认为0（此参数当mode=0时使用，支持排序）
	 * @param mCallBack
	 *            回调函数
	 * @param mTargetClass
	 *            返回对象类，如果返回json则为null
	 ...
	 *            BaseVO.TYPE_BEAN_LIST=3 BaseVO.TYPE_JSON=4
	 */
	public void friendFansList(Context context, String format, int reqnum,
			int startindex, int mode, int install, int sex,
			HttpCallback mCallBack, Class<? extends BaseVO> mTargetClass,
			int resultType) {
		ReqParam mParam = new ReqParam();
		mParam.addParam("scope", "all");
	 ...
	 *            返回数据的格式（json或xml）
	 * @param names
	 *            其他人的帐户名列表，用逗号“,”分隔，如aaa,bbb（最多30个，可选）
	 * @param fopenids
	 *            其他人的的用户openid列表，用“_”隔开，例如：
	 *            B624064BA065E01CB73F835017FE96FA_B624064BA065E01CB73F835017FE96FB
	 ...
	 *            BaseVO.TYPE_BEAN_LIST=3 BaseVO.TYPE_JSON=4
	 */
	public void friendCheck(Context context, String format, String names,
			String fopenids, int flag, HttpCallback mCallBack,
			Class<? extends BaseVO> mTargetClass, int resultType) {
		ReqParam mParam = new ReqParam();
		mParam.addParam("scope", "all");
		mParam.addParam("clientip", Util.getLocalIPAddress(context));
	 ...
	 *            BaseVO.TYPE_BEAN_LIST=3 BaseVO.TYPE_JSON=4
	 */
	public void getIntimateFriends(Context context, String format, int reqnum,
			HttpCallback mCallBack, Class<? extends BaseVO> mTargetClass,
			int resultType) {
		ReqParam mParam = new ReqParam();
		mParam.addParam("scope", "all");
